...
# Requires the VL model to be downloaded first.
#
# Usage:
#   bash nodes/ubuntu26-node1-server/game-server/serve/serve-vl.sh [GPU] [PORT]
#
# Default: GPU=2, port=8100
#
# Supports: Qwen2.5-VL-7B-Instruct, InternVL2-8B, etc.
...
VL_MODEL_DIR="/data/work/models/Qwen/Qwen2.5-VL-7B-Instruct"
SGLANG_IMAGE="voipmonitor/sglang:test-cu132"
CACHE_DIR="/data/cache/sglang_jit"
mkdir -p "$CACHE_DIR"

CONTAINER_NAME="sglang-vl-${PORT}"
...
if [[ ! -f "$VL_MODEL_DIR/config.json" ]]; then
    ...
    echo "  bash scripts/lib/download-model.sh Qwen/Qwen2.5-VL-7B-Instruct" >&2
    exit 1
fi
