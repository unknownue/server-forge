...
class AnthropicServing:
    ...
    async def handle_messages(
        ...
        logger.info(
            "Anthropic request: model=%s max_tokens=%s stream=%s messages=%s system=%s",
            request.model,
            request.max_tokens,
            request.stream,
    ...
    def _convert_to_chat_completion_request(
        ...
        def _extract_text_from_message(msg) -> str:
            """Extract plain text from a message's content, regardless of format."""
        ...
        # Accumulate extra system text from role=system messages
        extra_system_parts = []

        # Convert messages
        for msg in anthropic_request.messages:
            if msg.role == "system":
                extra_system_parts.append(_extract_text_from_message(msg))
                continue
            ...
            for block in msg.content:
                if block.type == "text" and block.text:
                    ...
                    # Skip internal reasoning blocks when they appear in
                    # conversation history (e.g. Claude Code retries).
                    continue
        ...
        # Merge system-role message text into the system prompt
        if extra_system_parts:
            extra_text = "\n\n".join(extra_system_parts)
            if openai_messages and openai_messages[0]["role"] == "system":
                openai_messages[0]["content"] += "\n\n" + extra_text
            else:
                openai_messages.insert(0, {"role": "system", "content": extra_text})
