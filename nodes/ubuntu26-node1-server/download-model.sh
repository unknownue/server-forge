...
#
# Usage:
#   bash nodes/ubuntu26-node1-server/download-model.sh                    # download all registered models
#   bash nodes/ubuntu26-node1-server/download-model.sh [MODEL_ID]         # download a specific model
...
# ── Model registry ──
# Format: "MODEL_ID|REVISION|FORMAT"
# FORMAT is optional (defaults to safetensors).
#   safetensors — safetensors weights + common configs/tokenizer
#   gguf        — all GGUF files + common configs/tokenizer
#   gguf:<quant> — GGUF matching <quant> + common (e.g. gguf:Q4_K_M)
#   full        — no filtering
#   "-"         — explicit default (same as safetensors)
#   <glob> ...  — custom allow_patterns
MODELS=(
    "Qwen/Qwen3.6-27B|main"
    "Qwen/Qwen3.6-27B-FP8|main"
    "unsloth/Qwen3.6-27B-NVFP4|main"
    "unsloth/Qwen3.6-27B-MTP-GGUF|main|gguf:Q4_K_M"
    # "some/model|v1.0|full"                                        # no filtering
    # "some/model|main|*.safetensors config.json tokenizer.*"        # custom globs
)

# ── Ensure huggingface_hub is available ──
if ! python3 -c "import huggingface_hub" 2>/dev/null; then
    echo "ERROR: huggingface_hub not installed." >&2
    echo "Run: sudo bash nodes/ubuntu26-node1-server/provision/install-packages.sh" >&2
    exit 1
fi
